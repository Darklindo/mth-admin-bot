...
A blacklist usa cache em memória, recebe mensagens normais e editadas e apaga em lote, com fallback individual. Ela cobre texto, fotos, vídeos, GIFs, stickers, documentos, áudios, vozes e outras mensagens que o Telegram permitir excluir. Ao cadastrar um alvo, as reações recentes desse usuário são removidas pela Bot API e novas reações são removidas assim que reportadas. Mensagens antigas que o bot não recebeu, mensagens protegidas ou operações sem permissão do Telegram continuam fora do alcance da Bot API.
