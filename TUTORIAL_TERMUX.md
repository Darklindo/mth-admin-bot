...
```text
.help
.jt ID/@username       # somente owner; autorizar neste grupo
.jt off ID/@username   # somente owner; revogar neste grupo
.jt list [página]      # somente owner; listar autorizados
.blacklist
.blacklist list
.blacklist list 2
.unblacklist
...
O `.jtbypass` aceita somente URLs iniciadas em `alpharede.com` ou `monteolympus.com` e tenta obter até oito redirecionamentos HTTP(S) públicos localmente; se necessário, usa o RedirectCheck público como fallback sem chave. O bot retorna a URL final observada como link clicável, sem executar JavaScript ou baixar arquivos. Ele não contorna captcha, login, anúncios obrigatórios, monetização, tokens protegidos ou proteção anti-bot; quando o destino exigir interação no navegador, o bot informa que não encontrou um redirecionamento público. O fallback externo pode registrar temporariamente a URL consultada conforme a política de privacidade do serviço; não envie links com dados sigilosos. O resultado não é uma garantia de que o conteúdo esteja disponível.
